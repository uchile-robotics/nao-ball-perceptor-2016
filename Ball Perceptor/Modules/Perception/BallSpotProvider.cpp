...
bool BallSpotProvider::getBallSpot(BallSpot& ballSpot) const
{
  ...
  ballSpot.height = float(std::abs(lower - upper)); //Here we assigned a height to the region
  ...
  ballSpot.width = float(std::abs(left - right)); //Here we assigned a width to the region
  LINE("module:BallSpotProvider:ballSpotScanLines",
